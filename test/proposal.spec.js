...
describe("proposal", function() {
  ...
  it("should create new proposal", done => {
    const proposal = new Proposal({
      experimentTitle: "dave@example.com",
  ...
  it("should get a proposal by its id", done => {
    const proposal = new Proposal({
      experimentTitle: "proposal@example.com",
      briefSummary: "proposal"
    });
    proposal.save((error, newProposal) => {
      chai
        .request(app)
        .get(`/api//proposals/${newProposal.id}`)
        .end((error, response) => {
          response.should.have.status(200);
          response.body.should.be.a("object");
  ...
  it("should update a proposal by its id", done => {
    ...
    proposal.save((error, newProposal) => {
      chai
        .request(app)
        .put(`/api//proposals/${newProposal.id}`)
        .send({ experimentTitle: "experimentTitle@experimentTitle.com" })
        .end((error, response) => {
          response.should.have.status(200);
          done();
        });
  ...
  it("should delete a proposal by its id", done => {
    ...
    proposal.save((error, newProposal) => {
      chai
        .request(app)
        .del(`/api/proposals/${newProposal.id}`)
        .end((error, response) => {
          response.should.have.status(200);
          done();
        });
