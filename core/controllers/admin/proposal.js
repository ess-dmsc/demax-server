...
const ProposalRound = require('../../models/proposalRound.js');

exports.getProposalRounds = async function(request, response){
	try {
		const proposalRounds = await ProposalRound.find({});
		response.status(200).json(proposalRounds);
	} catch(error) {
		console.log(error);
...
exports.deleteProposalRound = async function(request, response) {
	try {
		await ProposalRound.findOneAndDelete({
			proposalRoundId: request.params.proposalRoundId
		});
		console.log(request.params.proposalRoundId);
		response.status(200).json(request.params.proposalRoundId + ' was successfully deleted.');
	} catch(error) {
		console.log(error);
		return response.status(400).json({
			error: error.message
...
exports.createNewProposalRound = async function(request, response) {
	try {
		const newProposalRound = request.body;
		await new ProposalRound(newProposalRound).save();
		console.log('Created new proposal round ' + newProposalRound.proposalRoundId);
		response.status(201).json(newProposalRound);
	} catch(error) {
		console.log(error);
		return response.status(400).json({
			error: error.message
...
exports.editProposalRound = async function(request, response) {
	try {
		await ProposalRound.findOneAndUpdate({
			proposalRoundId: request.params.proposalRoundId
		}, request.body);
		response.status(200).json(request.params.proposalRoundId + ' was successfully saved.');
	} catch(error) {
		console.log(error);
